...
public:
	...
	void prepare(const double sampleRate, const int maximumBlockSize, const int numInputChannels, const int numOutputChannels)
	{
		...
		const auto maxCh = juce::jmax(nChIn, nChOut);
	...
	void process(const juce::dsp::AudioBlock<const float>& inputBlock, juce::dsp::AudioBlock<float>& outputBlock)
	{
		const auto L = (int)inputBlock.getNumSamples();
		const auto numChIn = juce::jmin(static_cast<int> (inputBlock.getNumChannels()), nChIn);
		const auto numChOut = juce::jmin(static_cast<int> (outputBlock.getNumChannels()), nChOut);
		const auto maxNumChannels = juce::jmax(numChIn, numChOut);
		...
		while (notYetUsedAudioDataCount > 0 && notYetUsedAudioDataCount + L >= fftSize)
		{
			...
			{
				juce::FloatVectorOperations::multiply(fftInOutBuffer.getWritePointer(ch), // destination
				...
				juce::FloatVectorOperations::multiply(fftInOutBuffer.getWritePointer(ch, notYetUsedAudioDataCount), // destination
		...
		else  // all of the previous data used
		{
			...
			while (L - dataOffset >= fftSize)
			{
				for (int ch = 0; ch < numChIn; ++ch)
				{
					juce::FloatVectorOperations::multiply(fftInOutBuffer.getWritePointer(ch),
		...
		{
			juce::FloatVectorOperations::copy(outputBlock.getChannelPointer(ch), outputBuffer.getReadPointer(ch), L);
		...
		for (int ch = numChOut; ch < outputBlock.getNumChannels(); ++ch)
			juce::FloatVectorOperations::clear(outputBlock.getChannelPointer(ch), L);
...
private:
	...
	{
		juce::dsp::WindowingFunction<float>::fillWindowingTables(window.data(), fftSize, juce::dsp::WindowingFunction<float>::WindowingMethod::hann, false);
...
protected:
	...
	juce::AudioBuffer<float> fftInOutBuffer;
	const int fftSize;
	const int hopSize;
...
private:
	...
	juce::AudioBuffer<float> notYetUsedAudioData;
